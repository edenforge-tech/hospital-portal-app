...
public class Schedule
{
    ...
    public Schedule(CounsellingService service)
    {
        _service = service;
    }
    ...
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "counselling/{id}/schedule")]
        HttpRequestData req,
        string id)
    {
        try
        {
            ...
            await _service.Schedule(guid, body.ScheduledDate, body.PerformedBy);

            var response = req.CreateResponse(HttpStatusCode.OK);
            await response.WriteAsJsonAsync(new { message = "Scheduled.", scheduledDate = body.ScheduledDate });
            return response;
        }
