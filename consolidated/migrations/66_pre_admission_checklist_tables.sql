-- ============================================================================
-- Migration 66: Pre-Admission Checklist Template System
-- Purpose: Dynamic, configurable checklist replacing hardcoded 8-item list.
--          Supports per-practice rules: patient type, surgery category, age.
-- Date: 2026-03
-- ============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Checklist templates (one per tenant+patient_type+surgery_category combo)
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS pre_admission_checklist_templates (
    ...
    -- Applicability rules (NULLs = applies to all)
    patient_type        VARCHAR(50),    -- Cash|Insurance|CoPay|ESH|CGHS|Arograshree|SGHS|Camp|Railway|Free|NULL(=all)
    surgery_category    VARCHAR(100),   -- Cataract|PHACO|LASIK|IOL|etc.; NULL=all
    min_patient_age     INTEGER,        -- NULL = no lower limit
    max_patient_age     INTEGER,        -- NULL = no upper limit
    applies_to_eye      VARCHAR(10),    -- RE|LE|Both|NULL(=all)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Checklist items (line items within a template)
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS pre_admission_checklist_items (
    ...
    item_key            VARCHAR(100)    NOT NULL,   -- machine-readable key (e.g. biometry_done)
    item_label          VARCHAR(200)    NOT NULL,   -- Display label
    description         TEXT,                       -- Helper text shown on hover

    -- Ownership / routing
    department_owner    VARCHAR(100),               -- Optometry|Lab|Cardiology|Billing|Admissions|OT|Anaesthesia
    department_color    VARCHAR(50),                -- Tailwind bg+text classes e.g. 'bg-teal-100 text-teal-700'

    -- Behaviour flags
    is_mandatory        BOOLEAN         NOT NULL DEFAULT TRUE,
    is_blocking         BOOLEAN         NOT NULL DEFAULT FALSE,  -- TRUE = cannot schedule until complete
    applies_if_age_below INTEGER,                               -- Auto-add when patient age < this value
    requires_document   BOOLEAN         NOT NULL DEFAULT FALSE, -- TRUE = upload proof required
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Per-schedule completion tracking
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS ot_admission_checklist_completions (
    id                  UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id           UUID            NOT NULL REFERENCES tenant(id) ON DELETE CASCADE,
    schedule_id         UUID            NOT NULL REFERENCES ot_booking_schedule(id) ON DELETE CASCADE,
    ...
    completed_by_dept   VARCHAR(100),                           -- which dept marked it
    document_url        TEXT,                                   -- optional proof upload
    notes               TEXT,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. Seed: default "Universal" template for all tenants
-- ─────────────────────────────────────────────────────────────────────────────
DO $$
DECLARE
    v_tenant    RECORD;
...
BEGIN
    FOR v_tenant IN SELECT id FROM tenant WHERE deleted_at IS NULL LOOP
        -- Skip if universal template already seeded
        IF NOT EXISTS (
