...
try {
    ...
    $response = Invoke-RestMethod -Uri "http://localhost:7071/api/activation/send-otp" -Method POST -Body $body -ContentType "application/json" -TimeoutSec 5
...
Write-Host "Test 2: Auth Service Integration..." -ForegroundColor Yellow
try {
    ...
    $response = Invoke-RestMethod -Uri "http://localhost:5073/api/users/00000000-0000-0000-0000-000000000001/send-activation" -Method POST -Headers $headers -Body $body -TimeoutSec 15
    Write-Host "SUCCESS! Auth -> Notification integration working!" -ForegroundColor Green
