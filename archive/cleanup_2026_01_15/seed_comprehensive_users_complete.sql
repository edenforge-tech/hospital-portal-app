...
BEGIN
    ...
    RAISE NOTICE '✓ Updated admin user with complete data';
    
    -- ====================
    -- CREATE DOCTORS (3 per branch)
    -- ====================
    FOR v_branch_record IN (
        SELECT id, name FROM branch WHERE tenant_id = v_tenant_id AND deleted_at IS NULL ORDER BY created_at LIMIT 3
    ) LOOP
        ...
        FOR i IN 1..3 LOOP
            ...
            FOR v_dept_record IN (
                ...
                INSERT INTO user_department_access (
                    id, user_id, department_id, role_id, tenant_id, branch_id,
                    is_primary, access_type, status, approval_status,
                    effective_from, granted_at, granted_by,
    ...
    RAISE NOTICE '✓ Created doctors across all branches';
        ...
        FOR i IN 1..5 LOOP
            ...
            FOR v_dept_record IN (
                ...
                INSERT INTO user_department_access (
                    ...
                    gen_random_uuid(), v_user_id, v_dept_record.id, v_nurse_role_id, v_tenant_id, v_branch_record.id,
                    true, 'full', 'active', 'approved',
                    NOW(), NOW(), v_admin_user_id, NOW(), NOW(), v_admin_user_id, v_admin_user_id
                ) ON CONFLICT DO NOTHING;
            END LOOP;
        END LOOP;
    END LOOP;
    
    RAISE NOTICE '✓ Created nurses across all branches';
    ...
    RAISE NOTICE '✓ Created receptionists across all branches';
    ...
    IF v_lab_tech_role_id IS NOT NULL THEN
        FOR v_branch_record IN (
            ...
            END LOOP;
        END LOOP;
        RAISE NOTICE '✓ Created lab technicians across all branches';
    END IF;
    ...
    IF v_pharmacist_role_id IS NOT NULL THEN
        FOR v_branch_record IN (
            ...
            END LOOP;
        END LOOP;
        RAISE NOTICE '✓ Created pharmacists across all branches';
    END IF;
    ...
    IF v_counsellor_role_id IS NOT NULL THEN
        FOR v_branch_record IN (
            ...
            ON CONFLICT DO NOTHING;
        END LOOP;
        RAISE NOTICE '✓ Created counsellors across all branches';
    END IF;
